...
from ..utils import get_ood_measures, print_measures, print_measures_with_std
...
class StreetHazardsDataset(CustomDataset):
    ...
    def get_class_count(self, path='.'):
        ...
        filename = 'class_count_street_hazards_pixel.npy'
        with open(osp.join(path, filename), 'wb') as f:
            np.save(f, self.class_count_pixel)

        filename = 'class_count_street_hazards_semantic.npy'
        with open(osp.join(path, filename), 'wb') as f:
