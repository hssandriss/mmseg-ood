...
def diss(bel):
    ...
    for i in cls:
        score_j_bal = [
            bel[:, j] * Bal(bel[:, j], bel[:, i]) for j in cls[cls != i]
        ]
...
def print_measures(auroc, aupr, fpr, ece, logger=None, text='max_prob'):
    ...
    print_log(
        'FPR{:d}: {:.2f}'.format(int(100 * RECALL_LEVEL_DEFAULT), 100 * fpr),
        logger)
    print_log('AUROC: {:.2f}'.format(100 * auroc), logger)
    print_log('AUPR: {:.2f}'.format(100 * aupr), logger)


def print_measures_with_std(aurocs,
                            auprs,
                            fprs,
                            eces,
                            logger=None,
    ...
    print_log(
        'FPR{:d}: {:.2f} +/- {:.2f}'.format(
            int(100 * RECALL_LEVEL_DEFAULT), 100 * np.mean(fprs),
            100 * np.std(fprs)), logger)
    print_log(
        'AUROC: {:.2f} +/- {:.2f}'.format(100 * np.mean(aurocs),
                                          100 * np.std(aurocs)), logger)
    print_log(
        'AUPR: {:.2f} +/- {:.2f}'.format(100 * np.mean(auprs),
                                         100 * np.std(auprs)), logger)
