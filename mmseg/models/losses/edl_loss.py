...
def ce_edl_loss(one_hot_gt, alpha, num_classes, func, bis=False, with_var=False, reg=False, entr=False):
    ...
    return A, torch.zeros(1, device=A.device), torch.zeros(1, device=A.device), torch.zeros(
        1, device=A.device), torch.zeros(1, device=A.device)
...
def lam(epoch_num, total_epochs, annealing_start, annealing_step, annealing_method, annealing_from):
    if annealing_method == 'step':
        annealing_coef = torch.min(
            ...
            torch.tensor(max(epoch_num - annealing_from, 0) / annealing_step, dtype=torch.float32))
        ...
        annealing_coef = torch.min(torch.tensor(1.0, dtype=torch.float32),
                                   torch.tensor(epoch_num / (total_epochs / 2), dtype=torch.float32)) * 0.1
    elif annealing_method == 'exp':
        annealing_coef = .1 * torch.min(
            annealing_start * torch.exp(-torch.log(annealing_start) /
                                        (annealing_step - 1) * epoch_num), torch.tensor(1.0, dtype=torch.float32))
...
class EDLLoss(nn.Module):
    ...
    def __init__(self,
        ...
        for epoch in range(self.total_epochs):
            self.lam_schedule.append(
                lam(epoch, self.total_epochs, self.annealing_start, self.annealing_step, self.annealing_method,
                    self.annealing_from))
    ...
    def forward(self, pred, target, weight=None, avg_factor=None, reduction_override=None, ignore_index=255):
        ...
        if self.loss_name.endswith('mse'):  # Eq. 5 MSE
            ...
            A, B, C, D, E = ce_edl_loss(one_hot_gt,
                                        alpha,
                                        self.num_classes,
                                        func=torch.log,
                                        bis=False,
