...
class BllViBaseDecodeHead(BaseModule, metaclass=ABCMeta):
    """Base class for BllViBaseDecodeHead.
    Args:
    ...
    def __init__(self,
        ...
        super(BllViBaseDecodeHead, self).__init__(init_cfg)
        ...
        self.conv_seg = nn.Conv2d(channels, self.out_channels, kernel_size=1).train(False)
        if dropout_ratio > 0:
    ...
    def build_density_estimator(self):
        if not self.vi_use_lower_dim:
            ...
            assert isinstance(self.vi_latent_dim, int), "When using lower dim in density, 'vi_latent_dim' is required !"
        ...
        if self.density_type == 'flow':
            if self.flow_type in ('householder_flow', 'planar_flow', 'radial_flow', 'sylvester_flow', 'naf_flow',
                ...
                self.density_estimation = DensityEstimation(dim=self.latent_dim,
            ...
            self.density_estimation = DensityEstimation(self.latent_dim, self.density_type)
        else:
            raise NotImplementedError
    ...
    def update_z0_params(self):
        ...
        if self.density_estimation.density_type in ('flow', 'cflow'):
            ...
            self.density_estimation.base_dist = tdist.MultivariateNormal(self.density_estimation.z0_mean,
        ...
        for loss_decode in losses_decode:
            if loss_decode.loss_name not in loss:
                ...
                if loss_decode.loss_name.startswith('loss_edl'):
                    # load
                    logs = loss_decode.get_logs(seg_logit, seg_label, self.ignore_index)
                    loss.update(logs)
            else:
    ...
    def __init__(self, dim, density_type='flow', **kwargs):
        ...
        if density_type == 'flow':
            ...
            if self.flow_type == 'radial_flow':
                ...
                transforms = [Sylvester(input_dim=self.dim, count_transforms=8) for _ in range(self.flow_length)]
            ...
            self.prior_density = tdist.MultivariateNormal(loc=torch.zeros(self.dim),
            ...
            if self.flow_type == 'conditional_radial_flow':
                transforms = [
                    ConditionalRadial(DenseNN(self.feat_dims, [self.feat_dims, self.feat_dims], [self.dim, 1, 1]))
                    for _ in range(self.flow_length)
                ]
            elif self.flow_type == 'conditional_planar_flow':
                transforms = [
                    ConditionalPlanar(DenseNN(self.feat_dims, [self.feat_dims, self.feat_dims], [self.dim, 1, 1]))
                    for _ in range(self.flow_length)
                ]
            elif self.flow_type == 'conditional_householder_flow':
                transforms = [
                    ConditionalHouseholder(DenseNN(self.feat_dims, [self.feat_dims, self.feat_dims], [self.dim, 1, 1]))
                    for _ in range(self.flow_length)
                ]

            elif self.flow_type == 'conditional_naf_flow':
                transforms = [
                    conditional_neural_autoregressive(input_dim=self.dim, context_dim=self.feat_dims)
                    for _ in range(self.flow_length)
                ]
            else:
                raise NotImplementedError
    ...
    def forward_flow(self, x, feats=None):
        ...
        for i in range(len(self.flow)):
            if isinstance(self.flow[i], ConditionalTransformModule):
                ...
                x_next = torch.cat((self.flow[i](x[:, self.multi_scale_masks[i]]), x[:, ~self.multi_scale_masks[i]]),
                                   dim=-1)
