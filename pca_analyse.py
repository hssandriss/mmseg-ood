...
flow_samples_2xnaf_sig_1xfc = joblib.load('flow_dist_2xnaf_sig+1xfc.pkl')
w_samples_2xnaf_sig_1xfc = joblib.load('proj_w_2xnaf_sig+1xfc.pkl')

transformed_initial_flow = PCA(n_components=4).fit_transform(np.concatenate(
    (initial_samples, flow_samples_4radial_2fc_relu, flow_samples_4sylv_1fc, flow_samples_2xnaf_sig_1xfc), axis=0))

pca = PCA(n_components=4)
w_samples = pca.fit_transform(np.concatenate((w_samples_4radial_2fc_relu, w_samples_4sylv_1fc, w_samples_2xnaf_sig_1xfc), axis=0))
...
color = np.array(
    ...
    ["Flow Distribution (2xnaf_sig+1xfc)" for _ in range(5000)] +
    ["Proj Weights (4radial+fc+relu+fc)" for _ in range(5000)] +
    ["Proj Weights (4sylv+1xfc)" for _ in range(5000)] +
    ["Proj Weights (2xnaf_sig+1xfc)" for _ in range(5000)]
)
labels = {
...
fig.update_traces(diagonal_visible=False, marker=dict(size=5, opacity=0.3))
fig.write_html(f'pca_4cp_naf.html')
